...
class MusicCog(commands.Cog, name="Música"):
    ...
    def get_queue_data(self, guild_id: int):
        ...
        elapsed = time.time() - start_time if current and start_time > 0 else 0
    ...
    async def cleanup(self, guild):
        ...
        self.queues.pop(guild_id, None); self.current_song.pop(guild_id, None); self.loop_states.pop(guild_id, None); self.start_times.pop(guild_id, None); self.history.pop(guild_id, None)
        await self.manager.broadcast(guild_id, self.get_queue_data(guild_id))
    ...
    async def search_song(self, query):
        ...
        try:
            with yt_dlp.YoutubeDL(opts) as ydl: data = await loop.run_in_executor(None, lambda: ydl.extract_info(query, download=False))
        except Exception as e: print(f"Erro no yt-dlp: {e}"); return None
    ...
    async def _add_to_queue_and_play(self, requester, text_channel, query: str):
        ...
        vc = guild.voice_client or await requester.voice.channel.connect()
    ...
    def _create_np_embed(self, guild_id):
        ...
        if start_time and song_info.get('duration'):
            elapsed = min(time.time() - start_time, song_info['duration'])
    ...
    async def pause_resume_from_web(self, guild_id: int):
        ...
        await self.broadcast_queue_update(guild_id); return action
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, commands.MissingRequiredArgument): return await ctx.send(f"Faltou um argumento! Uso correto: `{ctx.prefix}{ctx.command.name} {ctx.command.signature}`")
        print(f"Erro no comando '{ctx.command}' por '{ctx.author}': {error}"); await ctx.send(f"Ocorreu um erro. 😥\n`{type(error).__name__}: {error}`")
